...
<script type="text/javascript"
        src="//dapi.kakao.com/v2/maps/sdk.js?appkey=6427a2da1670b1b5f26b5608136a6892&libraries=services,clusterer,drawing"></script>
<%--<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=6427a2da1670b1b5f26b5608136a6892&libraries=services"></script>--%>
<script>
    $(function () {
        calculateDistance();
        ...
        <div class="mobile-view">
            <hearder id="header" class="fixed-top border border-black mobile-view bg-white shadow">
                ...
                <div style="width: 10%; float: left;">
                    <button type="button" class="col btn" onclick="back();"><i id="btn-back"
...
<%--                        <!--카카오 지도 열기 & 길찾기-->--%>
<%--                        <a id="kakao-location"--%>
<%--                           href="https://map.kakao.com/link/to/${store.name},${storeMarker.xLocation},${storeMarker.yLocation}"--%>
<%--                           title="길찾기"><i class="bi bi-map-fill"></i></a>--%>
<%--                        <!--현재 내 위치로 이동 버튼-->--%>
<%--                        <button id="my-location" onclick="mylocation();" title="현재 내 위치로 이동"><i--%>
<%--                                class="bi bi-geo-alt-fill"></i></button>--%>
<%--                        <!--가게 위치로 이동 버튼-->--%>
<%--                        <button id="store-location" onclick="storeLocation();" title="가게 위치로 이동"><i--%>
<%--                                class="bi bi-shop"></i></button>--%>
<%--                        000 님의 제보--%>
<%--                        <form id="storeNameBoxForm">--%>
<%--                            <input id="storeNameBox" class="h2 text-center" name="storeName" value="${store.name}"--%>
<%--                                   readonly>--%>
<%--                        </form>--%>
<%--                        &lt;%&ndash;                                <h2>${store.name}</h2>&ndash;%&gt;--%>
<%--                        <div class="center">--%>
...
<%--                    </div>--%>
                            ...
                            <div id="content-main-board" class="border card shadow">
                                ...
                                <div>
                                    <form name="favoriteForm" id="favoriteForm" action="/store/like">
                                        <button id="kakaotalk-sharing-btn" href="javascript:;" type="button"
                                                class="btn">
                                            <i class="bi bi-share"></i>
                                            공유하기
                                        </button>
                        ...
                        <div id="large-right">
                            <form id="edit-info" name="edit-info" method="post" action="/store/edit-info">
                                ...
                                <div id="content-information">
                                    ...
                                    <!--가게 정보 update 날짜-->
                                    <!--Todo : 업데이트 날짜 DB 연동-->
                                    <span id="updateInfo"><fmt:formatDate pattern="yyyy-MM-dd"
                                                                          value="${store.regDate}"></fmt:formatDate> 업데이트</span>
                                    <h4>가게 정보</h4>
                                    <div class="shadow border card" id="info-card">
                                        <div class="row">
                                            <div class="col-3 subtitle" style="margin: auto 0;font-weight: 600">출몰 시기
                                            </div>
                            ...
                            <div id="content-review">
                                <!--리뷰 작성하기 버튼-->
                                <a href="#main-row">
                                    <button id="btn-review" type="button" class="btn" onclick="reviewWrite();">리뷰작성하기
                                    </button>
                                </a>
                                ...
                                <div id="review-container">
                                    ...
                                    <c:forEach var="review" items="${reviews}">
                                        <div class="shadow border card review">
                                            <span class="createDate-review"><fmt:formatDate pattern="yyyy-MM-dd"
                                                                                            value="${review.regiDate}"></fmt:formatDate></span>
                                            <div class="row">
                                            ...
                                            <form action="/store/review-delete">
                                                <div class="row">
                                                    ...
                                                    <!--Todo : user 정보 가져와서 작성한 사람만 삭제할 수 있도록 수정하기-->
                                                        ...
                                                        <%--                                            </c:if>--%>
                                                    <button type="submit"
                                                            style="border: 0; outline: 0; display:inline; float: left; background-color: white; width: 30px; margin-right: 20px;">
                                                        <i class="bi bi-trash-fill col-2" style="margin: auto;"></i>
                                                    </button>
                                                ...
                                                <div class="row" style="height: 300px;overflow-y: scroll;">
                                                    <c:forTokens var="image" items="${token}" delims=";">
                                                        <img class="review-img"
                                                             src="${contextPath}/resources/upload/${image}" height="300"
        ...
        <div id="black-bg" style="z-index: 99;">
            <div id="review-write-container" class="card shadow bg-white" style="visibility: hidden;">
                <!--취소 버튼-->
                <button type="button" class="btn" onclick="reviewCancel();"><i id="review-cancel"
                ...
                <form id="review-form" action="/store/review" enctype="multipart/form-data" method="post"
                    ...
                    <div id="photos">
                        <button type="button" id="add-photo-button" class="btn btn-primary" onclick="addPhoto()">사진 추가
                        </button>
                        <button type="button" id="remove-photo-button" class="btn btn-warning d-none"
                                onclick="removePhoto()">사진 삭제
                        </button>
                    </div>
        ...
        <form action="/store">
            ...
            <div style="z-index: 100; position: relative; margin: auto;">
                <div id="visitFinished-modal" class="card shadow bg-white" style="visibility:hidden">
                    ...
                    <iframe src="https://embed.lottiefiles.com/animation/42183" width="100%"
